#!/usr/bin/env python
# -*- coding:utf8 -*-
# @TIME   : 2020/4/17 4:46 PM
# @Author : ChenLiuyin
# @File   : GridWorldEnvironment.py
...
class Grid(object):
    ...
    def _update_name(self):
        self.name = "X{0}-Y{1}-Z{2}".format(self.x, self.y, self.z)

    def __str__(self):
        return "name:{5}, x:{0}, y:{1}, z:{2}, type:{3}, value{4}".format(self.x,
                                                                   self.y,
                                                                   self.z,
                                                                   self.type,
                                                                   self.reward,
    ...
    def reset(self):
        ...
        for x in range(self.n_height):
            for y in range(self.n_width):
                for z in range(self.n_direction):
...
class GridWorldEnv(gym.Env):
    ...
    def step(self, action):
        ...
        if new_y == 0 and self.direc == 2:
            observation = 1  # red
        elif new_y == self.n_height-1 and self.direc == 0:
            observation = 2  # orange
        elif new_x == 0 and self.direc == 1:
            observation = 3  # blue
        elif new_x == self.n_width - 1 and self.direc == 3:
            observation = 4  # yellow
        else:
            observation = 0

        # 提供格子世界所有的信息在info内
        info = {"x": new_x, "y": new_y, "direction": self.direc, "grids": self.grids}
        return self.state, self.direc, self.reward, done, info, observation, suc
    ...
    def reset(self):
        ...
        self.direc = self.start_direction
        return self.state, self.direc
    ...
    def render(self, mode='human', close=False):
        ...
        all_width = self.n_width + 2
        all_height = self.n_height + 2

        # 如果还没有设定屏幕对象，则初始化整个屏幕具备的元素。
        if self.viewer is None:
            from gym.envs.classic_control import rendering
            self.viewer = rendering.Viewer(all_width * u_size, all_height * u_size)
            ...
            for x in range(all_width):
                for y in range(all_height):
                    ...
                    if (x == 0 and y != 0 and y != all_height-1):
                        rect.set_color(0, 1, 1)  # 蓝
                    elif (x == all_width - 1 and y != 0 and y != all_height-1):
                        rect.set_color(1, 1, 0)  # 黄
                    elif (y == 0):
                        rect.set_color(1, 0, 0)  # 红
                    elif (y == all_height-1):
                        rect.set_color(1, 0.5, 0)  # 橘色
                    else:
                        rect.set_color(0.9, 0.9, 0.9)  # 灰
                    ...
                    if self.start[0] == x-1 and self.start[1] == y-1:
                        ...
                        pass
            # 绘制个体, draw agent
            self.agent = rendering.make_polygon([(0, 0), (u_size, 0), (u_size*0.5, u_size)], filled=True)
            #self.agent = rendering.make_polygon([(p_size, p_size), (p_size * 2, p_size), (p_size * 1.5, p_size * 2)], filled=True)
            self.agent.set_color(0, 0, 0)
        ...
        if(z == 0):
            #print("up")
            self.agent_trans.set_rotation(2 * np.pi)
            self.agent_trans.set_translation((x + 1) * u_size, (y + 1) * u_size)
        if(z == 1):
            #print("left")
            self.agent_trans.set_rotation(0.5 * np.pi)
            self.agent_trans.set_translation((x + 2) * u_size, (y + 1) * u_size)
        if(z == 2):
            #print("down")
            self.agent_trans.set_rotation(1 * np.pi)
            self.agent_trans.set_translation((x + 2) * u_size, (y + 2) * u_size)
        if(z == 3):
            #print("right")
            self.agent_trans.set_rotation(1.5 * np.pi)
            self.agent_trans.set_translation((x + 1) * u_size, (y + 2) * u_size)
