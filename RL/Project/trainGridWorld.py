...
def trainLinear(maxiter=20,wp=(6*5,6*1),lamuda=0.00,printOut=True):
    ...
    dataloaderVAl = torch.utils.data.DataLoader(scadaValDataset, batch_size=5000,
                                                shuffle=True, num_workers=int(0))
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        if minloss > (loss / c):
            ...
            if lamuda==0:
                ...
                torch.save(state, '%s/GWRLLinearWT%d.pth' % (outf, int(predL / 6)))
            else:
...
print('Linear')
lossT,lossV,acc=trainLinear(maxiter=50,wp=(5*6,5*5),lamuda=0.00,printOut=True)
with open('95GWRes','wb') as f:
