    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        for i, (x,y,lb) in enumerate(dataloader):
            ...
            for tt in range(t):
                ...
                if ght.shape[0]<4:
                    continue
                loss3+=(torch.sum((ght[0,:]-ght[1,:])**2)+torch.sum((ght[0,:]-ght[2,:])**2)+
                        torch.sum((ght[1,:]-ght[2,:])**2)+torch.sum((ght[0,:]-ght[3,:])**2)+
                        torch.sum((ght[1,:]-ght[3,:])**2)+torch.sum((ght[2,:]-ght[3,:])**2))
                knum=list(np.where(classP==t)[0])
                for kok in knum:
                    for kok2 in knum:
                        loss4+=torch.sum((ckD[kok]-ckD[kok2])**2)
            # y=y.long()
            # l1loss=l1Norm(model)
            loss1/=k0
            loss2/=(k0*(k0+1)/2)
            ...
            loss=loss1*0+loss2*0+loss3*0+loss4*1
            loss.backward()
...
# print('Linear')
lossT,lossV,acc=trainLinear(maxiter=20,wp=(10*2,10*2),lamuda=0.00,printOut=True)
# with open('TradTWRes','wb') as f:
#     pickle.dump((lossT,lossV,acc),f)
# with open('TradTWRes','rb') as f:
...
axs[1].plot(acc)
plt.show()
f.savefig('TradTWRes.png', dpi=600, bbox_inches='tight')
